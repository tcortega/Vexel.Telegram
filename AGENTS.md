...
* Routing (T3/T5/T6/T7): dual-attr Option A - `[Handler]` + `[Command("name")]` / `[Callback("key")]` / `[InlineQuery("trigger"|"")]` / `[ChosenInlineResult("key")]`. Generator seeds FAWMN on `HandlerAttribute`; command/callback/inline/chosen maps keep route-attr types; **flow step map** includes every `[Handler]` with empty/single-string request (no route attr required). Emits `Add{Assembly}Telegram()` registering a `TelegramRouteContribution` (frozen command + callback + inline + chosen + flow step maps + binders that `GetRequiredService<X.Handler>()`). `TelegramRouter` implements `IUpdateRouter` and runs before raw handlers in `UpdateDispatcher`.
* Binding convention (normative in `CommandKeyExtractor` / `CommandArgumentBinder` / `CallbackKeyExtractor` / `InlineQueryKeyExtractor`): command key from offset-0 `BotCommand` entity; `@Bot` suffix stripped/matched; args bind by request ctor (empty / single string / tokenized primitives; trailing string takes rest). Callback key = `CallbackQuery.Data` up to first `|`; suffix after binds as single string (or empty record ignores it). Inline query routes on query text only (never `InlineQuery.Id`): first whitespace token exact-matches a non-empty trigger, remainder trimmed binds as string; empty/unmatched → empty-trigger default with full query text. Chosen inline result uses ResultId `key|suffix` like callbacks. Flow step payload = `Message.Text ?? Caption ?? ""` (empty or single string). Keyboard helpers: `Keyboards.InlineKeyboardBuilder` / `CallbackData.Format` emit short `key` / `key|suffix` and enforce the 64-byte UTF-8 cap.
* Flow (T6): concrete `Flow` + `IFlowStore`/`MemoryFlowStore` (namespace `Vexel.Telegram.Handlers`, files under `Flow/`). Step key = `typeof(TNext).FullName`. Text precedence: leading-`/` commands win (built-in `/cancel` only if no user `[Command("cancel")]`), then armed step, else unrouted; unknown `/foo` mid-flow is a command miss (not consumed as step input). B3 lifecycle: success without re-arm auto-completes; exception keeps armed + generic Feedback reply; TTL default 15m; draft is JSON POCO only. Do not put a type named `Flow` inside namespace `*.Flow` (CA1724).
* B4 answer obligation: `IUpdateCompletionHook` runs after routers + raw handlers; `CallbackAnswerObligation` / `InlineAnswerObligation` discharge default empty `answerCallbackQuery` / empty `answerInlineQuery` (`cache_time: 0`) via scoped `Feedback` when nothing answered (matched throw, unrouted, or silent success). `Feedback.AnswerCallbackAsync` / `AnswerInlineAsync` are first-wins idempotent so app answers win. Analyzer rule IDs live in `src/Vexel.Telegram.Generators/AnalyzerReleases.*.md` - do not restate them here.
