...
* Routing (T3/T5): dual-attr Option A - `[Handler]` + `[Command("name")]` / `[Callback("key")]`. Generator seeds FAWMN on `HandlerAttribute`, keeps types that also have a Vexel route attr; emits `Add{Assembly}Telegram()` registering a `TelegramRouteContribution` (frozen command + callback maps + binders that `GetRequiredService<X.Handler>()`). `TelegramRouter` implements `IUpdateRouter` and runs before raw handlers in `UpdateDispatcher`.
* Binding convention (normative in `CommandKeyExtractor` / `CommandArgumentBinder` / `CallbackKeyExtractor`): command key from offset-0 `BotCommand` entity; `@Bot` suffix stripped/matched; args bind by request ctor (empty / single string / tokenized primitives; trailing string takes rest). Callback key = `CallbackQuery.Data` up to first `|`; suffix after binds as single string (or empty record ignores it). Keyboard helpers: `Keyboards.InlineKeyboardBuilder` / `CallbackData.Format` emit short `key` / `key|suffix` and enforce the 64-byte UTF-8 cap.
* B4 answer obligation: `IUpdateCompletionHook` runs after routers + raw handlers; `CallbackAnswerObligation` discharges a default empty `answerCallbackQuery` via scoped `Feedback` when nothing answered (matched throw, unrouted, or silent success). `Feedback.AnswerCallbackAsync` is first-wins idempotent so app answers win. Analyzer rule IDs live in `src/Vexel.Telegram.Generators/AnalyzerReleases.*.md` - do not restate them here.
