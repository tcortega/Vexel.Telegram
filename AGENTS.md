...
* Binding convention (normative in plan/report): command key from offset-0 `BotCommand` entity; `@Bot` suffix stripped/matched; args bind by request ctor (empty / single string / tokenized primitives; trailing string takes rest). VEX0001 missing `[Handler]`, VEX0003 bad command name, VEX0004 unbindable shape, VEX0005 duplicate key.
