...
* Client dispatch: `VexelClient` long-polls via Telegram.Bot `ReceiveAsync`; `UpdateScheduler` runs ordered-per-chat lanes (bounded await-backpressure, idle eviction); `IUpdateDispatcher` is the routed → On* → raw seam (raw-only in T2); handler faults are isolated per update/handler and never kill the lane or polling loop.
* Lane keys: message chat id; callback `Message?.Chat.Id ?? From.Id`; inline/chosen `From.Id`; otherwise global (`UpdateLaneKey`).
* Unit tests live in `tests/Vexel.Telegram.Tests` (xunit); run with `dotnet test`.
