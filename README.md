...
| `Vexel.Telegram.Generators` | Roslyn generator + analyzers; consumed as an analyzer reference, never as a normal library |
...
Monorepo consumers must reference `Vexel.Telegram.Generators` themselves with `OutputItemType="Analyzer"`; analyzer project references do not flow transitively.
