...
public sealed class InlineRouterTests
{
	...
	{
		var logger = new RecordingLogger<InlineAnswerObligation>();
		var bot = new RecordingTelegramBotClient
		{
		...
		var obligation = new InlineAnswerObligation(bot, logger);
	...
	private static async Task UsingPipelineAsync(
		...
		Update update,
		Func<TelegramRouter, InlineAnswerObligation, RecordingTelegramBotClient, IServiceProvider, Update, Task> body)
	{
		var bot = new RecordingTelegramBotClient();
		var router = new TelegramRouter(contributions, bot, NullLogger<TelegramRouter>.Instance);
		var obligation = new InlineAnswerObligation(bot, NullLogger<InlineAnswerObligation>.Instance);
